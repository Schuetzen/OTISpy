*STPMN
     ...
     +   ITEMP,FD,FDLAST,FDSAVE,PV,PVMCD,PVNEW,PVPCD,PVSTP,PVTEMP,WT)
C
...
C  ARRAY ARGUMENTS
     ...
     +   PVNEW(N),PVPCD(N),PVSTP(N),PVTEMP(N),XM(IXM,M),WT(*)
      INTEGER
     +   IFAIL(N),ITEMP(N)
C
...
C
      PAR(J) = TEMP + STPCD
      CALL MDL(PAR, NPAR, XM, N, M, IXM, PVPCD, WT)
C
      PAR(J) = TEMP - STPCD
      CALL MDL(PAR, NPAR, XM, N, M, IXM, PVMCD, WT)
C
      PAR(J) = TEMP
C
...
C
      ...
      PAR(J) = TEMP + STP
C
      CALL MDL(PAR, NPAR, XM, N, M, IXM, PVSTP, WT)
C
      PAR(J) = TEMP
C
...
C
     ...
     +         TAUABS, STPLOW,
     +         STPMID, STPUP, ITEMP, FD, FDLAST, FDSAVE, PV, PVNEW, WT)
      ELSE
C
